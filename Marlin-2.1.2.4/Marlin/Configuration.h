...
#define STRING_CONFIG_H_AUTHOR "Sergio Gugliandolo" // Who made the changes.
 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 14.64f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1086f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1321f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.0998f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  ...
  Autotune results (tested with PLA settings and filament) [M306 T]
  #define MPC_BLOCK_HEAT_CAPACITY { 12.77f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1207f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1278f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.1825f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.

  Autotune results (tested with PETG settings and filament) [M306 T]
  #define MPC_BLOCK_HEAT_CAPACITY { 14.67f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1136f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1312f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.2187f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.

  Autotune results (tested with PETG settings and filament) [M306 T]
  #define MPC_BLOCK_HEAT_CAPACITY { 14.73f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1106f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1321f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.2188f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.

  Autotune results (tested with PETG settings and filament) [M306 T S1]
  #define MPC_BLOCK_HEAT_CAPACITY { 14.51f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1042f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1328f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.2196f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  
  Autotune results (tested with PETG settings and filament) [M306 T S2]
  #define MPC_BLOCK_HEAT_CAPACITY { 14.65f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1060f }        // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.1324f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.2186f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  */
  #endif
  ...
  #define MPC_MIN_AMBIENT_CHANGE 0.8f                 // (K/s) Modeled ambient temperature rate of change, when correcting model inaccuracies.
  #define MPC_STEADYSTATE 0.4f                        // (K/s) Temperature change rate for steady state logic to be enforced.
...
#define PIDTEMPBED
  ...
  */
  /* Autotune results
  90°: <M303 E-1 C8 S90>
  Recv: #define DEFAULT_bedKp 111.40
  Recv: #define DEFAULT_bedKi 21.59
  Recv: #define DEFAULT_bedKd 383.30

  60°: <M303 E-1 C8 S60>
  Recv: #define DEFAULT_bedKp 82.67
  Recv: #define DEFAULT_bedKi 15.53
  Recv: #define DEFAULT_bedKd 293.46
  */
 ...
 *     O-- FRONT --+
 */
#define NOZZLE_TO_PROBE_OFFSET { 42, -47, -2.45 }
